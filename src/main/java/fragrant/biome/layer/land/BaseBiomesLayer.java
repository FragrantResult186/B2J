...
public class BaseBiomesLayer extends IntBiomeLayer {
	...
	private int sampleOld(int center) {
		...
		Biome[] biomeList = this.getVersion().isOlderOrEqualTo(MCVersion.v1_1_0) ? OLD_BIOMES_DEFAULT_1_1 : OLD_BIOMES;
		Biome oldBiome = biomeList[this.nextInt(biomeList.length)];
		if(this.getVersion().isOlderOrEqualTo(MCVersion.v1_8_1)) {
