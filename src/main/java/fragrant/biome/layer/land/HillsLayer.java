	...
	public int sample(int x, int y, int z) {
		...
		if(toHills) {
			...
			if(i == Biomes.DESERT.getId()) {
				...
				l = this.getVersion().isOlderOrEqualTo(MCVersion.v1_6_2) ? Biomes.OCEAN.getId() : Biomes.DEEP_OCEAN.getId();
				...
				l = this.getVersion().isOlderOrEqualTo(MCVersion.v1_6_2) ? Biomes.MOUNTAINS.getId() : Biomes.WOODED_MOUNTAINS.getId();
			...
			if(l != i) {
				...
				if(Biome.areSimilar(biomesLayer.get(x - 1, y, z), b, this.getVersion())) m++;
				if(Biome.areSimilar(biomesLayer.get(x, y, z + 1), b, this.getVersion())) m++;
				if(this.getVersion().isOlderOrEqualTo(MCVersion.v1_6_0)) {
					if(m == 4) return l;
