...
public abstract class BiomeSource {
	private StaticNoiseSource staticNoiseSource;
	...
	public StaticNoiseSource getStaticNoiseSource() {
        if (staticNoiseSource == null) {
            staticNoiseSource = new StaticNoiseSource(worldSeed);
        }
	...
	public BPos locateNearestBiome(int centerX, int centerY, int centerZ, int radius, Collection<Biome> biomes, BedrockRandom rand) {
		return this.locateBiome(centerX, centerY, centerZ, radius, 1, biomes, rand, true);
	}

	public BPos locateBiome(int centerX, int centerY, int centerZ, int radius, Collection<Biome> biomes, BedrockRandom rand) {
	...
	public BPos locateBiome(int centerX, int centerY, int centerZ, int radius, int increment,
                            Collection<Biome> biomes, BedrockRandom rand, boolean checkByLayer) {
		centerX >>= 2;
		centerZ >>= 2;
		centerY >>= 2;
		radius >>= 2;
				...
				for(int ox = -depth; ox <= depth; ox += increment) {
					...
					if(biomes.contains(this.getBiomeForNoiseGen(x, centerY, z))) {
						if(pos == null || rand.nextInt(bound + 1) == 0) {
							...
							if(getVersion().isOlderOrEqualTo(MCVersion.v1_12_0)) {
								bound++;
							}
						}
						if(getVersion().isNewerOrEqualTo(MCVersion.v1_13_0)) {
							++bound;
						}
					}
				}
	...
	public BPos locateBiome12(int posX, int posZ, int radius, Collection<Biome> biomes, BedrockRandom rand) {
