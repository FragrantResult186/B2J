...
import fragrant.biome.layer.land.*;
import fragrant.biome.layer.noise.NoiseLayer;
import fragrant.biome.layer.scale.ScaleLayer;
import fragrant.biome.layer.scale.SmoothScaleLayer;
import fragrant.biome.layer.shore.EaseEdgeLayer;
import fragrant.biome.layer.shore.EdgeBiomesLayer;
import fragrant.biome.layer.temperature.ClimateLayer;
import fragrant.biome.layer.water.*;
...
public class OverworldBiomeSource extends LayeredBiomeSource<IntBiomeLayer> {
	...
	public OverworldBiomeSource(MCVersion version, long worldSeed, int biomeSize, int riverSize, boolean useDefault1_1) {
		...
		if(this.getVersion().isOlderThan(MCVersion.v1_8_0)) {
			throw new UnsupportedVersion(this.getVersion(), "overworld biomes");
		}
		if(this.getVersion().isOlderThan(MCVersion.v1_8_1)) {
	...
	protected void build() {
		...
		if(this.getVersion().isNewerOrEqualTo(MCVersion.v1_7_0)) {
		...
		if(this.getVersion().isNewerOrEqualTo(MCVersion.v1_0_0)) {
		...
		this.layers.add(this.base = new LandLayer(this.getVersion(), this.getWorldSeed(), this.getVersion().isNewerOrEqualTo(MCVersion.v1_0_0) ? 4L : 3L, this.base));
		...
		if(this.getVersion().isOlderThan(MCVersion.v1_13_0)) {
		...
		if(this.getVersion().isNewerOrEqualTo(MCVersion.v1_1_0)) {
		...
		for(int i = 0; i < this.biomeSize; i++) {
			...
			if((this.getVersion().isNewerOrEqualTo(MCVersion.v1_1_0) && (i == 1 || (this.biomeSize == 1 && this.getVersion().isNewerOrEqualTo(MCVersion.v1_8_0))))
				|| (this.getVersion().isOlderOrEqualTo(MCVersion.v1_0_0) && i == 0 && this.getVersion().isNewerOrEqualTo(MCVersion.v1_0_0))) {
			...
			if(i == 1 && this.getVersion().isBetween(MCVersion.v1_1_0, MCVersion.v1_6_2)) {
		...
		this.river = this.stack(
			this.getVersion().isNewerOrEqualTo(MCVersion.v1_7_0) ? 1000L : 1002L, NORMAL_SCALE,
			this.getVersion().isOlderThan(MCVersion.v1_13_0) ? this.river : this.noise,
			this.getVersion().isNewerOrEqualTo(MCVersion.v1_8_0) ? riverSize : biomeSize
		);
		...
		if(this.getVersion().isNewerOrEqualTo(MCVersion.v1_13_0)) {
			// ocean chains
