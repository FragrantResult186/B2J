...
import fragrant.biome.biome.surface.builder.BadlandsSurfaceBuilder;
import fragrant.biome.biome.surface.builder.BasaltDeltasSurfaceBuilder;
import fragrant.biome.biome.surface.builder.SoulSandValleySurfaceBuilder;
import fragrant.biome.layer.cache.FloatLayerCache;
...
/**
 * This class is specific to what we call block noises, they are only called per block and should be aggressively cached, we made sure to not separation them around to
...
public class StaticNoiseSource {
	...
	public static final OctaveSimplexNoiseSampler TEMPERATURE_NOISE = new OctaveSimplexNoiseSampler(new BedrockRandom(1234L), IntStream.of(0));
	// used for frozen ocean
	public static final OctaveSimplexNoiseSampler FROZEN_TEMPERATURE_NOISE = new OctaveSimplexNoiseSampler(new BedrockRandom(3456L), IntStream.of(-2, -1, 0));
	// used for flowers
	public static final OctaveSimplexNoiseSampler BIOME_INFO_NOISE = new OctaveSimplexNoiseSampler(new BedrockRandom(2345L), IntStream.of(0));
	...
	public Pair<OctaveSimplexNoiseSampler, OctaveSimplexNoiseSampler> getNoises() {
		if(this.noises == null) {
			BedrockRandom rand = new BedrockRandom((int) this.getWorldSeed());
	...
	public List<OctavePerlinNoiseSampler> getValleyNoise() {
		...
		if(this.valleyNoise == null) {
			...
			for(int i = 0; i < maxBlocksFloor + maxBlocksCeiling; i++) {
				OctavePerlinNoiseSampler sampler = CACHED_VALLEY_NOISE.computeIfAbsent(this.getWorldSeed() + i, key -> new OctavePerlinNoiseSampler(new BedrockRandom(key), Collections.singletonList(-4)));
	...
	public List<OctavePerlinNoiseSampler> getPatchNoise() {
		if(this.patchNoise == null) {
			...
			this.patchNoise.add(CACHED_PATCH_NOISE.computeIfAbsent(this.getWorldSeed() + souldSandSize, key -> new OctavePerlinNoiseSampler(new BedrockRandom(key), Collections.singletonList(0))));
			this.patchNoise.add(CACHED_PATCH_NOISE.computeIfAbsent(this.getWorldSeed() + basaltSize, key -> new OctavePerlinNoiseSampler(new BedrockRandom(key), Collections.singletonList(0))));
	...
	public OctavePerlinNoiseSampler getNetherForestsNoise() {
		if(this.netherForestsNoise == null) {
			BedrockRandom rand = new BedrockRandom(this.getWorldSeed());
