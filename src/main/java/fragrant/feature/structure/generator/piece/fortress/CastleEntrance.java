...
public class CastleEntrance extends Fortress.Piece {

    public CastleEntrance(int pieceId, ChunkRand rand, BlockBox boundingBox, BlockDirection facing) {
        super(pieceId);
    ...
    public static CastleEntrance createPiece(List<Fortress.Piece> pieces, ChunkRand rand, int x, int y, int z, BlockDirection facing, int pieceId) {
        BlockBox box = BlockBox.rotated(x, y, z, -5, -3, 0, 13, 14, 13, facing.getRotation());
        return Fortress.Piece.isHighEnough(box) && Fortress.Piece.getNextIntersectingPiece(pieces, box) == null ? new CastleEntrance(pieceId, rand, box, facing) : null;
    }
}