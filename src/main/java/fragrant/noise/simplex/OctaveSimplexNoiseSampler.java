package fragrant.noise.simplex;
...
public class OctaveSimplexNoiseSampler implements NoiseSampler {
	...
	public OctaveSimplexNoiseSampler(BedrockRandom random, int octaveCount) {
	...
	public OctaveSimplexNoiseSampler(BedrockRandom rand, IntStream octaves) {
		this(rand, octaves.boxed().collect(Collectors.toList()));
	}

	public OctaveSimplexNoiseSampler(BedrockRandom rand, List<Integer> octaves) {
		octaves = octaves.stream().sorted(Integer::compareTo).collect(Collectors.toList());

		if(octaves.isEmpty()) {
			throw new IllegalArgumentException("Need some octaves!");
		}
		int start = -octaves.getFirst();
		int end = octaves.getLast();
