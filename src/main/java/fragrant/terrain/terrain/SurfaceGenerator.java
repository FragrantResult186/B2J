...
import static fragrant.terrain.utils.MathHelper.*;
...
public abstract class SurfaceGenerator extends TerrainGenerator {
	...
	private final Map<Integer, double[]> noiseColumnCache = new HashMap<>();
	private final Map<Integer, Block[]> columnCache = new HashMap<>();
	private final Map<Integer, Block[]> jigsawColumnCache = new HashMap<>();
	private final Map<Integer, Block[]> biomeColumnCache = new HashMap<>();
	private final Map<Integer, Block[]> biomeJigsawColumnCache = new HashMap<>();
	private final Map<Integer, Integer> chunkSeeds = new HashMap<>();
	private final Map<Integer, Integer> jigsawChunkSeeds = new HashMap<>();
	...
	public SurfaceGenerator(BiomeSource biomeSource,
		...
		if(version.isOlderThan(MCVersion.v1_16_0)) {
			// those lines can be simplified to setting the lacunarity correctly to the lowest (FIXME)
			this.minLimitPerlinNoise = new OctavePerlinNoiseSampler(this.random, IntStream.of(16));
			this.maxLimitPerlinNoise = new OctavePerlinNoiseSampler(this.random, IntStream.of(16));
			this.mainPerlinNoise = new OctavePerlinNoiseSampler(this.random, IntStream.of(8));
		} else {
		...
		if(version.isOlderOrEqualTo(MCVersion.v1_13_2)) {
			...
			scaleNoise = new OctavePerlinNoiseSampler(this.random, IntStream.of(10));
		} else {
			scaleNoise = null;
		...
		if(version.isOlderThan(MCVersion.v1_16_0)) {
			this.depthNoise = new OctavePerlinNoiseSampler(this.random, IntStream.of(16));
		} else {
	...
	protected void sampleNoiseColumn(double[] buffer, int x, int z) {
		...
		for(int y = 0; y < this.noiseSizeY(); ++y) {
			...
			if(version.isNewerOrEqualTo(MCVersion.v1_16_0)) {
	...
	public void applyBedrock(int x, int z, BiFunction<Integer, Integer, Block[]> columnProvider, Map<Integer, Block[]> cacheProvider, Map<Integer, Integer> seedProvider) {
		int chunkX = x >> 4;
		int chunkZ = z >> 4;
		...
		// get the last seed
		Integer seed = seedProvider.get(getKey(maxChunkX, maxChunkZ));
		// should not fail
		if(seed == null) return;
		ChunkRand rand = new ChunkRand(seed);
	...
	private int getKey(int x, int z) {
		return (int) (((x & 0xFFFFFFFFL) << 32) | (z & 0xFFFFFFFFL));
	}

	public Optional<Block> getBlockAt(int x, int y, int z) {
		// long key = ((((long) y) & 0x3fff) << 50)  |((((long) blockX) & 0x1FFFFFF) << 25) | (((long) centerZ) & 0x1FFFFFF);
	 ...
	 * @param x      the blockX world position
	 * @param z      the centerZ world position
	 * @param rand   the specific rand for this column
	 */
	private void replaceBiomeBlocks(Block[] buffer, int x, int z, ChunkRand rand) {
		int y = (int) fragrant.terrain.utils.MathHelper.clamp(this.getHeightOnGround(x, z), this.getMinWorldHeight(), this.getMaxWorldHeight() - 1);
	 ...
	 *
	 * @param x              the world blockX position
	 * @param z              the world centerZ position
	 ...
	 */
	public Block[] generateBiomeColumnBefore(int x, int z, BiFunction<Integer, Integer, Block[]> columnProvider, Map<Integer, Block[]> cacheProvider, Map<Integer, Integer> seedProvider) {
		int chunkX = x >> 4;
		int chunkZ = z >> 4;
		...
		for(int X = 0; X < 16; X++) {
			for(int Z = 0; Z < 16; Z++) {
                ...
                int key = this.getKey(posX, posZ);
                Integer seed = seedProvider.get(key);
				if(seed == null) {
	...
	public double applyJigsawToNoise(double noise, BPos bPos, List<Pair<Supplier<Integer>, BlockBox>> jigsawBoxes, List<BPos> jigsawJunction) {
		noise = fragrant.terrain.utils.MathHelper.clamp(noise / 200.0D, -1.0D, 1.0D);
	...
	protected double[] getDepthAndScale(int x, int z) {
		...
		if(this.biomeSource.getVersion().isNewerOrEqualTo(MCVersion.v1_16_0)) {
...
//			return Math.min(noise, 1.0D) * 0.006640625D;
	...
	private static double computeContribution(int x, int y, int z) {
		...
		double d4 = -d1 * fragrant.terrain.utils.MathHelper.fastInvSqrt(d2 / 2.0D + d0 / 2.0D) / 2.0D;
		return d4 * d3;
	}
